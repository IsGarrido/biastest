...
class EvaluateEthnicities:

    def __init__(self) -> None:

        ethnicities_path = _project.data_path("wikipedia_ethnicities","list_of_contemporary_ethics_groups_dict.json")
        ethnicities = _pd.load_json(ethnicities_path)
        ...
        self.run(ethnicities)

    @log_time_with_counter
    def run(self, ethnicities: pd.DataFrame):

        model, tokenizer = _hf_model.load_model("bert-base-uncased", "bert-base-uncased")

        for index, row in ethnicities.iterrows():
            self.run_for_ethnicity(model, tokenizer, row['Ethnicity'])

    def run_for_ethnicity(self, model, tokenizer, ethnicity: str):
        sentence = ethnicity + " come from [MASK]."
        
        pipeline = FillMaskPipeline(model, tokenizer, top_k=10, device=model.device.index)
        predictions = pipeline(sentence)
        df = _pd.from_dict(predictions)
        print(df.head())
